...
exc = JsonSchemaValidationException('must be date-time', value='{data}', _rendered_path='data', definition='{definition}', rule='format')
...
exc = JsonSchemaValidationException('must be hostname', value='{data}', _rendered_path='data', definition='{definition}', rule='format')
@pytest.mark.parametrize('value, expected', [
    ('', exc),
    ('LDhsjf878&d', exc),
    ('bla.bla-', exc),
...
exc = JsonSchemaValidationException('must be custom-format', value='{data}', _rendered_path='data', definition='{definition}', rule='format')
